...
# Form implementation generated from reading ui file 'C:\Users\DELL\Desktop\new\AISearchingTechniquesV2.ui'
#
...
from greedy import Graph_greedy
...
class Ui_AISearchingTechniquesMainWindow(object):
        ...
        with open('test.txt', 'w') as f:
            ...
            if (graphType == "Undirectd Graph"):
                if searchType == "BFS":
                    ...
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path3, cost1, goals = graphucs.uniform_cost_search(start, goals)
                    if (traced_path3):
                        print('Path:', end=' ')
                        Graph_ucs.print_path(traced_path3, goals)
                        print('\nCost:', cost1)
                    ...
                    graphgreedy = Graph_greedy(directed=False)
                    for i in range(0, self.counter):
                        graphgreedy.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    graphgreedy.set_huristics(self.H)
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path4, cost2, goal = graphgreedy.greedy_search(start, goals)
                    if (traced_path4):
                        print('Path:', end=' ')
                        Graph_greedy.print_path(traced_path4, goal)
                        print('\nCost:', cost2)
                ...
                if searchType == "BFS":
                    ...
                    graphgreedy = Graph_greedy(directed=True)
                    for i in range(0, self.counter):
                        graphgreedy.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    graphgreedy.set_huristics(self.H)
                    start = self.StartNode_input.text()
    ...
    def setupUi(self, AISearchingTechniquesMainWindow):
        ...
        # self.ResetButton.setGeometry(QtCore.QRect(280, 780, 181, 41))
        # font = QtGui.QFont()
        # font.setPointSize(10)
    ...
    def retranslateUi(self, AISearchingTechniquesMainWindow):
        ...
        self.SearchTypecomboBox.setItemText(1, _translate("AISearchingTechniquesMainWindow", "UCS"))
        self.SearchTypecomboBox.setItemText(2, _translate("AISearchingTechniquesMainWindow", "DFS"))
        self.SearchTypecomboBox.setItemText(3, _translate("AISearchingTechniquesMainWindow", "Greedy"))
        ...
        # self.ResetButton.setText(_translate("AISearchingTechniquesMainWindow", "Reset"))
